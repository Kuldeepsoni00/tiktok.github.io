var move = 1;
var play = true;
$('table tr td').click(function () {
    if ($(this).text() == "" && play && move!="9"  ) {
        if ((move) % 2 == 1) {
            $(this).append("x");
            $(this).css("color", "#61892f");
        }
        else {
            $(this).append("0");
            $(this).css("color", "#e85a47");
        }
        move++;
        if (findwinner() != -1 && findwinner() != "") {
            if (findwinner() == "x") {
                $("body").append('<div class="winner"><span>Winner</span>x</div>');
                $("body").append('<button onclick="location.reload()">Play Again</button>');
                $(".winner").css('background-color', '#61892f')
                $('button').css('color', '#e85a47');
            }

            else {
                $("body").append('<div class="winner"><span>Winner</span>0</div>');
                $("body").append('<button onclick="location.reload()">Play Again</button>');
                $(".winner").css('background-color', '#e85a47')
                $('button').css('color', '#e85a47');
            }
            play = false;


        }
       

    }
    else if(move=="9" && findwinner() == -1){
        $(this).append("x");
        $(this).css("color", "#61892f");
        $("body").append('<div class="winner"><span>No Winner</span></div>');
        $("body").append('<button onclick="location.reload()">Play Again</button>');
        $(".winner").css('background-color', '#e85a47')
        $('button').css('color', '#e85a47');
    }
});

function findwinner() {
    sp1 = $('table tr:nth-Child(1) td:nth-child(1)').text();
    sp2 = $('table tr:nth-Child(1) td:nth-child(2)').text();
    sp3 = $('table tr:nth-Child(1) td:nth-child(3)').text();
    sp4 = $('table tr:nth-Child(2) td:nth-child(1)').text();
    sp5 = $('table tr:nth-Child(2) td:nth-child(2)').text();
    sp6 = $('table tr:nth-Child(2) td:nth-child(3)').text();
    sp7 = $('table tr:nth-Child(3) td:nth-child(1)').text();
    sp8 = $('table tr:nth-Child(3) td:nth-child(2)').text();
    sp9 = $('table tr:nth-Child(3) td:nth-child(3)').text();

    if (sp1 == sp2 && sp2 == sp3) {
        return sp3;
    }
    else if (sp1 == sp2 && sp2 == sp3) {
        return sp3;
    }
    if (sp4 == sp5 && sp5 == sp6) {
        return sp6;
    }
    if (sp7 == sp8 && sp8 == sp9) {
        return sp9;
    }
    if (sp1 == sp4 && sp4 == sp7) {
        return sp7;
    }
    if (sp2 == sp5 && sp5 == sp8) {
        return sp8;
    }
    if (sp3 == sp6 && sp6 == sp9) {
        return sp9;
    }
    if (sp1 == sp5 && sp5 == sp9) {
        return sp9;
    }
    if (sp3 == sp5 && sp5 == sp7) {
        return sp3;
    }

    return -1;



}
